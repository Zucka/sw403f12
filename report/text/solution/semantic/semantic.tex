...
The semantic analyser builds 3 tables. The Symbol table, containing the symbols that is in the slide. The Slide table, containing information about a slide. And the Error table containing information about errors made by the user
...
Errors made in the semantic analysis, are handled by not doing the command, and adding a description of the error in an Error table containing the slide number, and a description and the Value containing the error.\\
Listing \ref{LSTMakeError} creates an object in the ErrorList array, where the first dimension states which error number it is. The second dimension in cell 0 contains the information about what slide number the error has occured. In cell 1 the type of error is inserted, and in cell 2 is the Value of the error. An example of an error is made in listing \ref{LSTErrorlist}, where the the user tried to change the font color in slide number 2 to \texttt{SomeColor}, which is not a valid color.
...
\begin{lstlisting} [frame=single,caption= Errorlist, label=LSTErrorlist]
