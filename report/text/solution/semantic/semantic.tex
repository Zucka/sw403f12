...
This section focus on blocks. A block consists of 2 lines, a \texttt{begin} line and an \texttt{end} line.
...
In order for the compiler to apply a transition on a slide, the analyser has to check if the transition that is written exists in the database of transitions. If the transition doesn’t exist an error has to occur. If the transition does exist, then it continues without error.
