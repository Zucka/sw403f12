...
In order for the compiler to apply a transition on a slide, the analyser has to check if the transition that is written exists in the database. If the transition doesn’t exist an error has to occur. If the transition does exist, then it continues without error.
