...
\section{Keywords}
This is a list of reserved words in the language, where the words with @-prefix may not be used otherwise. Because @-prefix should state that an event is about to happen.
...
\texttt{FontChanges} could be font size, font type and so on, written in the language as \texttt{@font\_size} and \texttt{@font\_type} respectively. 
 \texttt{Value}, is the value that it is set to. \texttt{Type} is referring to which font class it will change, e.g. title, text, the keyword \texttt{global} in this matter means that all font classes are changed.
...
A star has to be set on each line that is going to be a bullet.
 ...
 \subsection{Hash tag (\#)}
\texttt{\#} expression starts a numeration list starting from 1, and for each line the has tag is used, the number is incremented. If the line after a hash tag line is not a hash tag line, the numeration list is ended and thus a new hash tag line would have it's numeration start with 1.
