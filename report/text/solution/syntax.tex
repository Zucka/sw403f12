...
Listing \ref{lst:BasicElemets} shows some of the basic elements in the language.
...
Each slide can contain a transition, these transitions are set as keywords, and is listed in listing \ref{lst:TransitionKeywords}
\begin{lstlisting}[frame=single, caption=Transition keywords, label=lst:TransitionKeywords]
...
\subsection{@setting}
This expression sets a new setting for a specific type, or globally. A generic example of this is shown in listing \ref{lst:SettingGeneric}.
\begin{lstlisting}[frame=single, caption=setting expression generic, label=lst:SettingGeneric]
