...
The requirements for the language is defined in the section ´´Language design''\ref{LanguageRequirements}. To begin with we need some keywords to make the language act different according to the order of these keywords, plus these keywords has to have a meaningfull name according to it's use/effect. Secondly we need to define in which order these keywords are allowed in, and what they mean in that particualer order.

\section{Keywords}
This is a list of reserved words in the language, whereas the ones with the @-prefix may not be used otherwise.
...
Another keyword is ``setting'', in this example the setting is set for the type ``title'' to a font size of 40 points. \\
A setting can be initiated outside, as well as inside, a slide. The difference is that, outside a slide the setting is applied to all the upcoming slides, if the setting is inside the slide, it is only applied to that slide. If no settings are set while making the slides, standard settings will be used.
...
Because there are no additinal settings to be set on this sentence, no pipe has to accure. Between the left curly and the potential pipe, settings can be applied to that specific sentence, like:\\
...
\texttt{@setting} changes a parameter for the slide or all upcomming slides.\\
