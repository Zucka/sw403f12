...
\noindent{Which states that a slide begins, with a transition ``fade'', and that the slide contains the text ``Hello World''. \\
...
\noindent{A way to use the title type can be as shown in the following example:} \\
...
\noindent{Because there are no additional settings to be set on this sentence, no pipe has to occur. \\
Besides the \texttt{title} type, there is the \texttt{subtitle} type, which makes a subtitle for the slide, which means that it will be placed beneath the title, per default with a smaller font size.}
...
\noindent{Between the left curly and the potential pipe, settings can be applied to that specific sentence, like:} \\
...
\noindent{The weight of the text can be set in two ways, the first looks like the way we just changed the font size:} \\
...
\noindent{A quicker way to set bold text is as follows:} \\
...
\begin{lstlisting}[frame=single, label=lst:Fast-NonFast]
...
\noindent{The \texttt{global} expression makes it possible to apply font weights globally. The weights could be font colour, font family, links, etc.). Furthermore, \texttt{global} overwrites normal text, titles, subtitles, image descriptions and URLs. \\
An example of the use of \texttt{global} can be seen beneath, in this case operating with \texttt{font\_weight} and \texttt{subtitle}:}
...
\noindent{The following is a larger example to demonstrate what is just used:} \\
...
\noindent{Settings do not have to be set at all, if they are not set, the default settings will be used.}
...
\noindent{Where a star symbolises a bullet, two bullets symbolises a bullet inside a bullet.
...
\noindent{Where a \# symbolises a number that is incrementing. Using two \# creates a sub numeration starting from one.}
...
\noindent{Where the URL, or file path, to the image is placed before the pipe and after the keyword ``@url''. The text after the pipe is the image text.}