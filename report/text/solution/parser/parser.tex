...
\begin{itemize}
	...
	\item The CFG should not be ambiguous either, so that we can make a parser for it. The CFG is (KOM GERNE MED FORESLAG TIL ÆNDRING AF ORDLYD) concluded to be LL(1) according to the section \ref{Parserstrategy}, the CFG is not ambiguous either, as per the definition of LL(1). (\cite{CaC} Chapter 5.1)
\end{itemize}
\newpage
\section{CFG}
