...
This section is based on the book ``Concepts of Programming Languages'' \cite{CoPL}. The design of the language is made using the language criteria in the table below. These criteria can be categorised into three primary categories; \textit{readability}, \textit{writability} and \textit{reliability}. These overall categories have a number of secondary categories, which is used for specifying the language design. Which secondary categories that affect the overall categories are listed in the table below.
...
Writability is another important criterion in designing a programming language. It refers to how easy it is to write programs using the language, in this programming language it should be easy to write and express slides. A programming language is usually easier to use, if it shares some characteristics with popular languages. However, a high writability ranking can also come from the language being simple to use. The writability is deemed ``very important'', because our language is a slide show programming language, and that a slideshow is only made once.
...
Portability is known as the ease with which programs can be implemented on another platform than the one they were originally developed for. \\
...
\noindent{In the programming language of this project, the orthogonality is deemed ``important'', because it is important that there are different ways to express text formatting - making it simpler and easier to use.} \\
\\ \\
\textbf{Expressivity} \\
...
be seen as an extension to writability, in that it can make it easier to express statements in a programming. In the developed programming language, expressivity is deemed ``important'' because there is focus on making the language as easy and convenient to write and express slide shows in.
\\ \\
\textbf{Readability} \\
...
In the developed programming language, readability is deemed ``less important'', because it is not as important as the writability or reliability, though it still has to be possible to read codes written in the language.
\\ \\
\textbf{Cost} \\
...
\noindent{In the programming language of this project, the cost is deemed ``less important'' because;}
\begin{itemize}
	\item It is not going to be taught to other programmers than the developers, because it is a proof of concept.
	\item The only slideshows that are going to be written in the language is written by the developers themselves.
