...
To find errors or bugs in the language or compiler a series of tests have been run to verify that a given input would resul9t in a viable output. 
...
\begin{lstlisting}[frame=single,caption=NISSE construct,label=lst:ltest0, language=nisse]
@begin{slide}
...
(because the font family is recognized as a \texttt{shortidentv1}). It is not only \texttt{Times New Roman} that does not work; it is every font family that contains space(s) in their names, which is because the compiler does not accept a \texttt{space} as a setting, in the family font.}
...
An example of a correct input is in listing \ref{lst:ltest11}. This is also a bug which will not be corrected at this time, but should be before initial release.
